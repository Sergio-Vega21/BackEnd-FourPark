...
module.exports = {
    async create(factura) {
        const { name, email, password } = factura;
    ...
    async update(id, user) {
        ...
        const result = await pool.query(
            'UPDATE factura SET correo_electronico = $1, id_reserva = $2, id_factura = $3, hora_inicio = $4, hora_fin = $5, fecha_inicio = $6, fecha_fin = $7, precio = $8 WHERE id = $4 RETURNING *',
            [correo, reserva, factura, horaInicio, horaFin, fechaInicio, fechaFin, precio]
        );
        return result.rows[0];
    },

    async delete(id) {
        const result = await pool.query('DELETE FROM factura WHERE id = $1 RETURNING *', [id]);
        return result.rows[0];
    }
};
